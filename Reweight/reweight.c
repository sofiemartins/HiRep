...
void read_cmdline(int argc, char *argv[]) {
    ...
    if (op > 0 && op < argc) { strcpy(output_filename, argv[op]); }

    if (ip > 0 && ip < argc) { strcpy(input_filename, argv[ip]); }

    if (cp > 0 && cp < argc) { strcpy(config_filename, argv[cp]); }
}
...
#ifdef REWEIGHT_THETA
    lprintf("REWEIGHT", 10, "old twisting angles = %1.4f %1.4f %1.4f %1.4f\n", rw_var.old_theta_t, rw_var.old_theta_x,
            rw_var.old_theta_y, rw_var.old_theta_z);
    lprintf("REWEIGHT", 10, "new twisting angles = %1.4f %1.4f %1.4f %1.4f\n", rw_var.new_theta_t, rw_var.new_theta_x,
            rw_var.new_theta_y, rw_var.new_theta_z);
#endif
