...
void read_gauge_field_openQCD(char filename[]) {
    ...
    error((fp = fopen(filename, "rb")) == NULL, 1, "read_gauge_field_openQCD", "Failed to open file for reading");

    error(fread_LE_int(size, 4, fp) != 4, 1, "read_gauge_field_openQCD",
                ...
                for (g[3] = 0; g[3] < GLB_Z; g[3]++) {
                    if ((g[0] + g[1] + g[2] + g[3]) % 2 == 1) {
                        for (mu = 0; mu < 4; mu++) {
                            ...
                            error(fread(test, sizeof(double), 2 * NG * NG, fp) != 2 * NG * NG, 1, "read_gauge_field_openQCD",
                ...
                for (g[3] = 0; g[3] < GLB_Z; g[3]++) {
                    if ((g[0] + g[1] + g[2] + g[3]) % 2 == 1) {
                        for (mu = 0; mu < 4; mu++) {
                            ...
                            if (g[0] == 0) { id = ipt(GLB_T - 1, g[1], g[2], g[3]); }
                            id = idn(id, mu);
...
void write_gauge_field_openQCD(char filename[]) {
    ...
    error((fp = fopen(filename, "wb")) == NULL, 1, "write_gauge_field_openQCD", "Failed to open file for writing");

    error(fwrite_LE_int(size, 4, fp) != 4, 1, "write_gauge_field_openQCD",
                ...
                for (g[3] = 0; g[3] < GLB_Z; g[3]++) {
                    if ((g[0] + g[1] + g[2] + g[3]) % 2 == 1) {
                        for (mu = 0; mu < 4; mu++) {
                            ...
                            error(fwrite_LE_double(test, 2 * NG * NG, fp) != 2 * NG * NG, 1, "read_gauge_field_openQCD",
                                  "Failed to read header from file");
