...
void read_gauge_field_fortran(char filename[]) {
    ...
    error((fp = fopen(filename, "rb")) == NULL, 1, "read_gauge_field_fortran", "Failed to open file for reading");

    error(fread(info, sizeof(float), 16, fp) != 16, 1, "read_gauge_field_fortran", "Failed to read header from file");
            ...
            for (g[3] = 0; g[3] < GLB_Z; g[3]++) {
                for (g[2] = 0; g[2] < GLB_Y; g[2]++) {
                    for (g[1] = 0; g[1] < GLB_X; g[1]++) {
                        ...
                        for (i = 0; i < NG; i++) {
                            for (j = 0; j < NG; j++) {
                                pu_gauge(ipt(g[0], g[1], g[2], g[3]), (mu + 1) % 4)->c[i + j * NG] =
                                    test[2 * (j + i * NG)] + I * test[2 * (j + i * NG) + 1];
                            }
                        }
                    }
                }
