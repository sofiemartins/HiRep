...
static int index_eolexi(int x0, int x1, int x2, int x3) {
    ...
    if ((y0 + y1 + y2 + y3) & 1) { ix += (GLB_T * GLB_X * GLB_Y * GLB_Z) / 2; }
...
void write_gauge_field_eolexi_BE(char filename[]) {
    ...
    if (PID == 0) {
        error((fp = fopen(filename, "wb")) == NULL, 1, "write_gauge_field_eolexi", "Failed to open file for writing");
        error(fwrite_BE_double((double *)eolexi_field, 4 * GLB_T * GLB_X * GLB_Y * GLB_Z * sizeof(suNg) / sizeof(double), fp) !=
                  4 * GLB_T * GLB_X * GLB_Y * GLB_Z * (int)sizeof(suNg) / (int)sizeof(double),
              1, "write_gauge_field_eolexi", "Failed to write gauge field to file");
        fclose(fp);
        free(eolexi_field);
    }
...
void read_gauge_field_eolexi_BE(char filename[]) {
    ...
    if (PID == 0) {
        error((fp = fopen(filename, "rb")) == NULL, 1, "read_gauge_field_eolexi", "Failed to open file for reading");
        error(fread_BE_double((double *)eolexi_field, 4 * GLB_T * GLB_X * GLB_Y * GLB_Z * sizeof(suNg) / sizeof(double), fp) !=
                  4 * GLB_T * GLB_X * GLB_Y * GLB_Z * (int)sizeof(suNg) / (int)sizeof(double),
              1, "read_gauge_field_eolexi", "Failed to read gauge field from file");
    }
...
#else
                    lprintf("IO", 20, "(%.2f , %.2f) ", creal(pu_gauge(ix, mu)->c[i * NG + j]),
                            cimag(pu_gauge(ix, mu)->c[i * NG + j]));
#endif //GAUGE_SON
                }
                lprintf("IO", 20, "]\n");
            }
...
void write_gauge_field_eolexi_LE(char filename[]) {
    ...
    if (PID == 0) {
        error((fp = fopen(filename, "wb")) == NULL, 1, "write_gauge_field_eolexi", "Failed to open file for writing");
        error(fwrite_LE_double((double *)eolexi_field, 4 * GLB_T * GLB_X * GLB_Y * GLB_Z * sizeof(suNg) / sizeof(double), fp) !=
                  4 * GLB_T * GLB_X * GLB_Y * GLB_Z * (int)sizeof(suNg) / (int)sizeof(double),
              1, "write_gauge_field_eolexi", "Failed to write gauge field to file");
        fclose(fp);
        free(eolexi_field);
...
void read_gauge_field_eolexi_LE(char filename[]) {
    ...
    if (PID == 0) {
        error((fp = fopen(filename, "rb")) == NULL, 1, "read_gauge_field_eolexi", "Failed to open file for reading");
        error(fread_LE_double((double *)eolexi_field, 4 * GLB_T * GLB_X * GLB_Y * GLB_Z * sizeof(suNg) / sizeof(double), fp) !=
                  4 * GLB_T * GLB_X * GLB_Y * GLB_Z * (int)sizeof(suNg) / (int)sizeof(double),
              1, "read_gauge_field_eolexi", "Failed to read gauge field from file");
    }
