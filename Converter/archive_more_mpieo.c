        ...
        for (g[1] = 0; g[1] < GLB_X; ++g[1]) {
            for (g[2] = 0; g[2] < GLB_Y; ++g[2]) {
                ...
                for (p[3] = 0; p[3] < NP_Z; ++p[3]) { /* loop over processors in Z direction */
                    ...
                    if (PID == 0) {
                        error(fwrite_LE_double(buff, (size_t)(bsize), fp) != (bsize), 1, "write_gauge_field",
...
void read_gauge_field_mpieo_LE(char filename[]) {
    ...
    if (PID == 0) {
        ...
        error(fread_LE_int(d, (size_t)(5), fp) != (5), 1, "read_gauge_field", "Failed to read gauge field geometry");
        ...
        for (g[1] = 0; g[1] < GLB_X; ++g[1]) {
            for (g[2] = 0; g[2] < GLB_Y; ++g[2]) {
                ...
                for (p[3] = 0; p[3] < NP_Z; ++p[3]) { /* loop over processors in Z direction */
                    ...
                    if (PID == 0) {
                        error(fread_LE_double(buff, (size_t)(bsize), fp) != (bsize), 1, "read_gauge_field",
