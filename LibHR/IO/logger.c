...
int logger_map(char *name, char *filename) {
    ...
    if (rd != 0) {
        ...
        if (strcmp(filename, rd->name) == 0) { return 0; }
    }
...
void logger_set_input(input_logger *logger) {
    if (logger->def_log_lvl == -1) { logger->def_log_lvl = 10; }
...
void logger_rmlevel(char *name) {
    ...
    if (name == 0) { return; /* nothing to do */ }

    rd = lfindname(levels, name);
    if (rd == 0) { return; /* no record found: already at defaut level */ }
