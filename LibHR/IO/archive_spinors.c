            ...
            for (g[2] = 0; g[2] < GLB_Y; ++g[2]) {
                ...
                for (p[3] = 0; p[3] < NP_Z; ++p[3]) { /* loop over processors in Z direction */
                    ...
                    if (pid != 0) { /* do send/receive only if the data is not on PID 0 */
                        ...
                        if (pid == PID) {
#ifndef NDEBUG
                            error(Z != (GLB_Z / NP_Z + ((p[3] < rz) ? 1 : 0)), -1, "write_spinor_field",
                    ...
                    if (PID == 0) {
                        error(fwrite(buff, sizeof(double), bsize, fp) != bsize, -1, "write_spinor_field",
        ...
        for (g[1] = 0; g[1] < GLB_X; ++g[1]) {
            for (g[2] = 0; g[2] < GLB_Y; ++g[2]) {
                ...
                for (p[3] = 0; p[3] < NP_Z; ++p[3]) { /* loop over processors in Z direction */
                    ...
                    if (PID == 0) {
                        error(fread(buff, sizeof(double), bsize, fp) != bsize, -1, "read_spinor_field",
                    ...
                    if (pid != 0) { /* do send/receive only if the data is not on PID 0 */
                        ...
                        if (PID == pid) {
#ifndef NDEBUG
                            error(Z != (GLB_Z / NP_Z + ((p[3] < rz) ? 1 : 0)), -1, "read_spinor_field",
                                  "Local lattice size mismatch!");
#endif
