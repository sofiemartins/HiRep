...
#undef _SWAP
    ...
    if (rot12 && rot23) {
        if (c[0] == c[1] && c[1] == c[2]) {
            ...
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = aaa_perm[w];
            }
        } else if (c[0] == c[1]) {
            WL_path[WL_npaths].nperms = 3;
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = aab_perm[w];
            }
        } else if (c[1] == c[2]) {
            WL_path[WL_npaths].nperms = 3;
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = abb_perm[w];
            }
        } else {
            WL_path[WL_npaths].nperms = 6;
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = abc_perm[w];
            }
        }
    } else if (rot12) {
        if (c[0] == c[1]) {
            ...
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = aaX_perm[w];
            }
        } else {
            WL_path[WL_npaths].nperms = 2;
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = abX_perm[w];
            }
        }
    } else if (rot23) {
        if (c[1] == c[2]) {
            ...
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = Xaa_perm[w];
            }
        } else {
            WL_path[WL_npaths].nperms = 2;
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = Xab_perm[w];
            }
        }
    } else if (rot13) {
        if (c[0] == c[2]) {
            ...
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = aXa_perm[w];
            }
        } else {
            WL_path[WL_npaths].nperms = 2;
            WL_path[WL_npaths].perm = amalloc(sizeof(int *) * WL_path[WL_npaths].nperms, ALIGN);
            for (int w = 0; w < WL_path[WL_npaths].nperms; w++) {
                WL_path[WL_npaths].perm[w] = aXb_perm[w];
            }
        }
    }

    lprintf("ARA WILSON LOOPS", 0, "c=( %d , %d , %d ) path added (nsteps = %d ; length = %d ; nperms = %d )\n", c[0], c[1],
            c[2], nsteps, WL_path[WL_npaths].length, WL_path[WL_npaths].nperms);
...
void WL_Hamiltonian_gauge(suNg_field *out, suNg_field *in) {
    ...
    if (COORD[0] != 0) {
        MPI_Status status;
        MPIRET(mpiret)
        MPI_Recv((double *)(buf_gtf[1]), /* buffer */
    ...
    if (COORD[0] == 0) {
        ...
        for (x = 0; x < X; x++) {
            for (y = 0; y < Y; y++) {
                for (z = 0; z < Z; z++) {
                    i = ipt(T - 1, x, y, z);
                    _suNg_times_suNg(buf_gtf[0][_WL_3VOL_INDEX(x, y, z)], buf_gtf[1][_WL_3VOL_INDEX(x, y, z)],
                                     *_FIELD_AT(ws_gtf[0], i));
                }
            }
        }
    }
    ...
    if (COORD[0] != NP_T - 1) {
        MPIRET(mpiret)
        MPI_Send((double *)(buf_gtf[0]), /* buffer */
                 (X * Y * Z) * sizeof(suNg) / sizeof(double), /* lenght in units of doubles */
                 MPI_DOUBLE, /* basic datatype */
                ...
                for (z = 0; z < Z; z++) {
                    ...
                    *_FIELD_AT(ws_gtf[1], i) = buf_gtf[1][_WL_3VOL_INDEX(x, y, z)];
                    for (int t = 1; t < T; t++) {
                        i = ipt(t, x, y, z);
                        j = ipt(t - 1, x, y, z);
                        _suNg_times_suNg(*_FIELD_AT(ws_gtf[1], i), buf_gtf[1][_WL_3VOL_INDEX(x, y, z)],
                                         *_FIELD_AT(ws_gtf[0], j));
                    }
                }
            }
        }
        ...
        if (mpiret != MPI_SUCCESS) {
            ...
            for (k = 0; k < NP_T - 1; ++k) {
                if (status[k].MPI_ERROR != MPI_SUCCESS) {
                    ...
                    lprintf("MPI", 0, "Req [%d] Source [%d] Tag [%] ERROR: %s\n", k, status[k].MPI_SOURCE, status[k].MPI_TAG,
                            mesg);
                }
            }
...
static void WL_parallel_transport(suNg *ret, const suNg_field *gf, int x, const int *path, const int length, const int perm[3],
                                  const int sign[3]) {
    suNg tmp;
    int inv[3];
...
void WL_correlators(double **ret, const suNg_field *gf, const suNg *poly, const int nsteps, const int *path, const int length,
                    const int perm[3], int sign[3]) {
    suNg tmp[2];
    int c[3];
    ...
    for (int s = 0; s < nsteps; s++) {
        if (s == 0) {
            for (int t = 0; t < T; t++) {
                for (int x = 0; x < X; x++) {
                    for (int y = 0; y < Y; y++) {
                        for (int z = 0; z < Z; z++) {
                            WL_parallel_transport(_FIELD_AT(ws_gtf[0], ipt(t, x, y, z)), gf, ipt(t, x, y, z), path, length,
                                                  perm, sign);
                ...
                for (int x0 = 0; x0 < X; x0++) {
                    for (int y0 = 0; y0 < Y; y0++) {
                        for (int z0 = 0; z0 < Z; z0++) {
                            ...
                            _suNg_times_suNg(tmp[0], buf_gtf[0][_WL_4VOL_INDEX(t, x0, y0, z0)],
                                             *_FIELD_AT(ws_gtf[0], ipt(t, x1, y1, z1)));
        ...
        for (int DT = 1; DT <= NP_T; DT++) {
            ...
            if (DT < NP_T) {
                ...
                MPIRET(mpiret)
                MPI_Isend((double *)(buf_gtf[0]), /* buffer */
                ...
                MPIRET(mpiret)
                MPI_Irecv((double *)(buf_gtf[2]), /* buffer */
                          (T * X * Y * Z) * sizeof(suNg) / sizeof(double), /* lenght in units of doubles */
                          MPI_DOUBLE, /* basic datatype */
                ...
                for (int t1 = 0; t1 < T; t1++) {
                    if (((COORD[0] + DT - 1) * T + t1) % GLB_T >= zerocoord[0] + t0) {
                        for (int x0 = 0; x0 < X; x0++) {
                            for (int y0 = 0; y0 < Y; y0++) {
                                for (int z0 = 0; z0 < Z; z0++) {
                                    _suNg_times_suNg_dagger(tmp[0], buf_gtf[0][_WL_4VOL_INDEX(t0, x0, y0, z0)],
                        ...
                        for (int x0 = 0; x0 < X; x0++) {
                            for (int y0 = 0; y0 < Y; y0++) {
                                for (int z0 = 0; z0 < Z; z0++) {
                                    ...
                                    _suNg_times_suNg(tmp[0], buf_gtf[0][_WL_4VOL_INDEX(t0, x0, y0, z0)],
                                                     poly[_WL_3VOL_INDEX(x1, y1, z1)]);
    ...
    for (int n = 0; n < WL_npaths; n++) {
        ...
        for (int p = 0; p < WL_path[n].nperms; p++) {
            for (int w = 0; w < 4; w++) {
                ...
                WL_correlators(tmp, HYP, Polyakov, WL_path[n].nsteps, WL_path[n].path, WL_path[n].length, WL_path[n].perm[p],
                               sign);
                for (int s = 0; s < WL_path[n].nsteps; s++) {
                    for (int t = 0; t < GLB_T; t++) {
                        WL[s][t] += tmp[s][t];
        ...
        for (int s = 0; s < WL_path[n].nsteps; s++) {
            ...
            for (int t = 0; t < GLB_T; t++) {
                lprintf("WILSON LOOPS", 0, "(T,dx,dy,dz,R,WL) = %d %d %d %d %.8e %.8e\n", t, (s + 1) * WL_path[n].c[0],
                        (s + 1) * WL_path[n].c[1], (s + 1) * WL_path[n].c[2],
                        (s + 1) * sqrt(WL_path[n].c[0] * WL_path[n].c[0] + WL_path[n].c[1] * WL_path[n].c[1] +
                                       WL_path[n].c[2] * WL_path[n].c[2]),
                        WL[s][t]);
            }
        }
    }
