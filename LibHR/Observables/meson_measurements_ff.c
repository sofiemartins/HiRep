...
void measure_spectrum_discon_ff_semwall(int nm, double *m, int nhits, int degree_hopping, int nhits_hopping, int conf_num,
    ...
    for (k = 0; k < nhits; ++k) {
        ...
        if (nhits_hopping > 0) {
            print_mesons(discon_correlators, -(2.0 * (double)nhits_hopping) / 2., conf_num, nm, m, GLB_T, 1, label);
        }
        if (nhits_hopping > 0) {
            print_mesons(triplet_discon_correlators, -(2.0 * (double)nhits_hopping) / 2., conf_num, nm, m, GLB_T, 1, label);
        }
    }
    ...
    for (k = 0; k < nhits; ++k) {
        create_noise_source_equal_eo(source);
        calc_propagator_ff_hopping_oe(prop, source, degree_hopping,
                                      4); //5 for order of hopping parameter expansion,  4 for spin dilution
        measure_mesons(triplet_discon_correlators, prop, source, nm, 0);
        measure_mesons(discon_correlators, prop, source, nm, 0);
