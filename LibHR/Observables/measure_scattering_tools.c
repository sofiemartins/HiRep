...
#define PROP \
    ...
    X(p_0)   \
    X(mp_0)
#define X(NAME) fun(prop->W_##NAME, srcp->src_##NAME, ndilute, tau);
    PROP
#undef X
#undef PROP
        /// \endcond
...
#define X(NAME) do_global_sum(mo->NAME, 1.0);
    R
#undef X
...
#define R \
    ...
    X(r3) \
    X(r4)
#define X(NAME) reset_mo(mo->NAME);
    R
#undef X
#undef R
        /// \endcond
                ...
                for (pz = 0; pz < pmax; ++pz) {
                    for (t = 0; t < GLB_T; ++t) {
                        fprintf(file, "%i %i %i %i %3.10e %3.10e \n", px, py, pz, t,
                                2 * (mo->corr_re[corr_ind(px, py, pz, pmax, t, 1, 0)]),
                                2 * (mo->corr_im[corr_ind(px, py, pz, pmax, t, 1, 0)]));
                    }
                }
            }
        }
        ...
        for (px = -pmax; px <= pmax; ++px) {
            for (py = -pmax; py <= pmax; ++py) {
                for (pz = -pmax; pz <= pmax; ++pz) {
                    for (t = 0; t < GLB_T; ++t) {
                        fprintf(file, "%i %i %i %i %3.10e %3.10e \n", px, py, pz, t,
                                4 * (mo->corr_re[INDEX(px, py, pz, pmax, t)]), 4 * (mo->corr_im[INDEX(px, py, pz, pmax, t)]));
                    }
                }
            }
        }
    ...
    for (px = 0; px < pmax; ++px)                                                                                              \
        for (py = 0; py < pmax; ++py)                                                                                          \
            for (pz = 0; pz < pmax; ++pz) {                                                                                    \
                ...
                for (int src = 0; src < numsources; src++) {                                                                   \
                    for (int t = 0; t < GLB_T; ++t) {                                                                          \
                        fprintf(f, "\t\t\t\t[%e,%e]", 2 * (molist[src]->STRUCT->corr_re[corr_ind(px, py, pz, pmax, t, 1, 0)]), \
                                2 * (molist[src]->STRUCT->corr_im[corr_ind(px, py, pz, pmax, t, 1, 0)]));                      \
                        if (t < GLB_T - 1) {                                                                                   \
                            fprintf(f, ",\n");                                                                                 \
                        } else {                                                                                               \
                            fprintf(f, "]");                                                                                   \
    ...
    for (px = 0; px < pmax; ++px)                                                                                     \
        for (py = 0; py < pmax; ++py)                                                                                 \
            for (pz = 0; pz < pmax; ++pz) {                                                                           \
                ...
                for (int src = 0; src < numsources; src++) {                                                          \
                    for (int t = 0; t < GLB_T; ++t) {                                                                 \
                        fprintf(f, "\t\t\t\t[%e,%e]", 4 * (molist[src]->STRUCT->corr_re[INDEX(px, py, pz, pmax, t)]), \
                                4 * (molist[src]->STRUCT->corr_im[INDEX(px, py, pz, pmax, t)]));                      \
                        if (t < GLB_T - 1) {                                                                          \
                            fprintf(f, ",\n");                                                                        \
                        } else {                                                                                      \
                            fprintf(f, "]");                                                                          \
...
void IO_json_p(struct mo_p *molist[], int numsources, char *path, char *cnfg_filename) {
    ...
    int pmax = 2;
    if (PID == 0) {
        sprintf(outfile, "%s/p_(%d,%d,%d)_n%s.json", path, molist[0]->p[0], molist[0]->p[1], molist[0]->p[2],
                strrchr(cnfg_filename, 'n') + 1);
        lprintf("IO_json_0", 0, "%s\n", outfile);
        f = fopen(outfile, "w+");
        fprintf(f, "{\n");
        JSON(pi, "pi");
...
void measure_pion_scattering_I0(double *m, int numsources, double precision, char *path, char *cnfg_filename, int seq_prop,
    ...
    for (int src = 0; src < numsources; ++src) {
        ...
        // "standard" two points : pi and rho
        for (int t = 0; t < GLB_T; ++t) {
            measure_mesons_core(prop_ts1[src][t], prop_ts1[src][t], source_ts1[t], pi1, 1, (ts + t) % GLB_T, 1, 0,
                                GLB_T); // this is summing over t
        }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                for (int t = 0; t < GLB_T; ++t) {
                    measure_mesons_core(prop_ts1[src][t], prop_ts1[src][t], source_ts1[t], rho1[i][j], 1, (ts + t) % GLB_T, 1,
                                        0, GLB_T);
                }
                ...
                for (int t = 0; t < GLB_T; ++t) {
                    for (int dt = 0; dt < GLB_T; ++dt) {
                        for (int alpha = 0; alpha < 4; alpha++) {
                            for (int beta = 0; beta < 4; beta++) { // dilution indices
                                A = Ralt_contract(prop_ts1[src1][t] + alpha, prop_ts1[src2][(t + dt) % GLB_T] + beta,
                                                  (t + ts1) % GLB_T);
                                B = Ralt_contract(prop_ts1[src2][(t + dt) % GLB_T] + beta, prop_ts1[src1][t] + alpha,
                                                  (t + dt + ts2) % GLB_T);

                                global_sum((double *)(&A), 2);
                                global_sum((double *)(&B), 2);

                                Ralt->corr_re[corr_ind(0, 0, 0, 1, (dt + ts2 - ts1 + GLB_T) % GLB_T, 1, 0)] +=
                                    creal(A * B) / (GLB_T * numsources * numsources);
                                Ralt->corr_im[corr_ind(0, 0, 0, 1, (dt + ts2 - ts1 + GLB_T) % GLB_T, 1, 0)] +=
                                    cimag(A * B) / (GLB_T * numsources * numsources);
                            }
                        }
                    }
                }
