...
static void WF_measure_and_store(suNg_field *V, storage_switch swc, data_storage_array **ret, int nmeas, int idmeas,
    ...
    local_variance(sp, V);
    double hsq_avg = sqnorm_scalar_field(sp) / (double)GLB_VOLUME;
    free_field(sp);

    E = WF_E(V);
    Esym = WF_Esym(V);
    lprintf("WILSONFLOW", 0,
            "WF (t,E,t2*E,Esym,t2*Esym,TC,h,h2_avg) = %1.8e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e\n", *t, E,
            *t * *t * E, Esym, *t * *t * Esym, TC, h, E / 12.0, sqrt(hsq_avg));
    if (swc == STORE) {
        idx[0] = idmeas - 1;
        idx[1] = 0;
