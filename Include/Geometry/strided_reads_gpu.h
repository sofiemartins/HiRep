...
__host__ __device__ __forceinline__ void read_gpu(int stride, SITE_TYPE *s, const FIELD_TYPE *in, size_t ix, int comp, int dim) {
...
template <typename REAL, typename SITE_TYPE> __device__ __forceinline__ void write_out_spinor_field(SITE_TYPE *r, SITE_TYPE *in, int ix) {
    write_gpu<REAL>(0, r, in, ix, 0, 1);
}

template <typename REAL, typename FIELD_TYPE, typename SITE_TYPE>
__host__ __device__ __forceinline__ void write_assign_gpu(int stride, SITE_TYPE *s, FIELD_TYPE *out, int ix, int comp, int dim) {
    const int field_dim = sizeof(FIELD_TYPE) / sizeof(REAL);
    const int n_components = sizeof(SITE_TYPE) / sizeof(REAL);
#ifdef FIXED_STRIDE
