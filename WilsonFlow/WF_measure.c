...
#define init_input_WF_meas(varname)                                                                \
    {                                                                                              \
        .read = { { "WF integrator type", "WF:integrator = %d", INT_T, &((varname).ittype) },      \
...
#define init_input_SF(varname)                                                               \
    {                                                                                        \
        .read = { { "SF background", "SF:background = %d", INT_T, &((varname).background) }, \
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(WF_var.configlist, "") != 0) {
        error((list = fopen(WF_var.configlist, "r")) == NULL, 1, "main [WF_measure.c]", "Failed to open list file\n");
    }
