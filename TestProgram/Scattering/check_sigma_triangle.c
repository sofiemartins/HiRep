                ...
                for (q14 = 0; q14 < LT; ++q14) {
                    for (q24 = 0; q24 < LT; ++q24) {
                        for (q34 = 0; q34 < LT; ++q34) {
                            ...
                            numerator = (af1[0] * af1[1] + af2[0][1]) * sin(mom[2].v[2]) +
                ...
                for (q14 = 0; q14 < LT; ++q14) {
                    for (q24 = 0; q24 < LT; ++q24) {
                        for (q34 = 0; q34 < LT; ++q34) {
                            ...
                            numerator =
                                (af1[0] * af1[1] * af1[2]) - af1[0] * (af2[1][2]) + af1[1] * (af2[0][2]) + af1[2] * (af2[0][1]);
...
int compare_2pt(meson_observable *mo, hr_complex *corr, int px, int py, int pz, int pmax, double tol) {
    ...
    for (int t = 0; t < GLB_T; t++) {
        ...
        if (fabs(num_re - ana_re) > tol || fabs(num_im - ana_im) > tol) {
            lprintf("TEST", 0, "Mismatch, t=%d, numeric = %e + I*(%e), analytic = %e + I*(%e)", t, num_re, num_im, ana_re,
                    ana_im);
            retval = 1;
        } else {
...
int main(int argc, char *argv[]) {
    ...
    for (int t = 0; t < GLB_T; t++) {
        ...
        Dtheo[t] = NF * disc_id(zero_p, m[0], GLB_X, GLB_T, t) *
                   0.5; // factor 2 is due to the eo normalisation of the stochastic sources.
        Ctheo[t] = NF * twopoint_id(zero_p, m[0], GLB_X, GLB_T, t) *
                   0.5; // source is not even/odd but the io2pt is called with a 0.5 normalisation
    }

    for (int i = 0; i < GLB_T; ++i) {
        lprintf("MAIN", 0, "T analytical T %e + 1I %e  numerical %e + 1I %e\n", creal(Ttheo[i]), cimag(Ttheo[i]),
                creal(Tstoch[i]), cimag(Tstoch[i]));
    }
    for (int i = 0; i < GLB_T; ++i) {
        lprintf("MAIN", 0, "diff analytical T %e + 1I %e, and relative %e  \n", creal(Ttheo[i]) - creal(Tstoch[i]),
                cimag(Ttheo[i]) - cimag(Tstoch[i]), (creal(Ttheo[i]) - creal(Tstoch[i])) / (creal(Ttheo[i])));
    }

    for (int i = 0; i < GLB_T; ++i) {
        lprintf("MAIN", 0, "Disc analytical Disc %e + 1I %e  numerical %e + 1I %e\n", creal(Dtheo[i]), cimag(Dtheo[i]),
                creal(Dstoch[i]), cimag(Dstoch[i]));
    }
    for (int i = 0; i < GLB_T; ++i) {
        lprintf("MAIN", 0, "diff analytical Disc %e + 1I %e, and relative %e  \n", creal(Dtheo[i]) - creal(Dstoch[i]),
                cimag(Dtheo[i]) - cimag(Dstoch[i]), (creal(Dtheo[i]) - creal(Dstoch[i])) / (creal(Dtheo[i])));
    }

    for (int i = 0; i < GLB_T; ++i) {
        lprintf("MAIN", 0, "Conn analytical C %e + 1I %e  numerical %e + 1I %e\n", creal(Ctheo[i]), cimag(Ctheo[i]),
                creal(Cstoch[i]), cimag(Cstoch[i]));
    }
    for (int i = 0; i < GLB_T; ++i) {
        lprintf("MAIN", 0, "diff analytical C %e + 1I %e, and relative %e  \n", creal(Ctheo[i]) - creal(Cstoch[i]),
                cimag(Ctheo[i]) - cimag(Cstoch[i]), (creal(Ctheo[i]) - creal(Cstoch[i])) / (creal(Ctheo[i])));
    }
