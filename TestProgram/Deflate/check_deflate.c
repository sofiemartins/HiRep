 ...
 *
 * NOCOMPILE = BC_T_SF_ROTATED || BC_T_SF || FERMION_THETA
 * 
 *******************************************************************************/
    ...
    while (1) {
        ...
        lprintf("MAIN", 0, "m %g n %d prec %g Saved %g\n", m[0], eig_var.nev, eig_var.omega1,
                (4 * NF * GLB_T * ((double)etime.tv_sec + (double)etime.tv_usec * 1e-6)) /
                    (4 * NF * GLB_T * ((double)e2time.tv_sec + (double)e2time.tv_usec * 1e-6) + (double)eig_time.tv_sec +
                     (double)eig_time.tv_usec * 1e-6));
