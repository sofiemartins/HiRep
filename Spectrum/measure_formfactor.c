...
int main(int argc, char *argv[]) {
    ...
    if (mes_var.ff_def_point) { lprintf("MAIN", 0, "Point for form factors sources\n"); }
    ...
    while (1) {
        ...
        if (mes_var.ff_fixed_point) {
            measure_formfactor_fixed(mes_var.ti, mes_var.tf, mes_var.dt, nm, m, mes_var.n_mom, i, mes_var.precision, DONTSTORE,
                                     NULL);
        }
