...
int main(int argc, char *argv[]) {
    ...
    if (mes_var.def_gfwall) { lprintf("MAIN", 0, "Gauge Fixed Wall Source\n"); }
    if (mes_var.ext_semwall) { lprintf("MAIN", 0, "Spin Explicit Method (SEM) wall sources on extended lattice\n"); }
    if (mes_var.ext_point) { lprintf("MAIN", 0, "Point sources on extended lattice\n"); }
    ...
    while (++i) {
        ...
        if (four_fermion_active == 0) {
            ...
            if (mes_var.def_point) { measure_spectrum_pt(tau, nm, m, mes_var.n_mom, i, mes_var.precision, DONTSTORE, NULL); }
            ...
            if (mes_var.def_point) { measure_spectrum_ff_pt(tau, nm, m, mes_var.n_mom, i, mes_var.precision, DONTSTORE, NULL); }
            if (mes_var.ext_semwall) {
                measure_spectrum_semwall_ff_ext(nm, m, mes_var.nhits_2pt, i, mes_var.precision, DONTSTORE, NULL);
            }
            if (mes_var.discon_semwall) {
                measure_spectrum_discon_ff_semwall(nm, m, mes_var.nhits_disc, mes_var.degree_hopping, mes_var.nhits_hopping, i,
